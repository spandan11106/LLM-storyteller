...
class MemoryManager:
    ...
    def __init__(self):
        ...
        self.turn_count = 0
        self.knowledge_graph = {}  # Our simple knowledge graph
        print("Memory Manager Initialized.")

    def _get_summary(self, text):
        """Generates a one-sentence summary of a turn for memory storage."""
        summary_prompt = (
            "Summarize the following RPG turn exchange in a single, concise sentence, "
            f"focusing on the key action or outcome:\n\n{text}"
        )
        try:
    ...
    def _extract_and_update_facts(self, text, user_input):
        ...
        # --- Quest Update Logic (Keyword Based) ---
...
- For relationships: {{"subject": "entity", "relation": "verb", "object": "entity"}}
- For properties: {{"entity": "entity", "property": "description"}}
            ...
            if isinstance(facts, list):
                ...
                for fact in facts:
                    if isinstance(fact, dict):
                        ...
                        if 'subject' in fact:
                            self.knowledge_graph[entity_name]['relationships'].append(fact)
                        elif 'entity' in fact:
                            self.knowledge_graph[entity_name]['properties'].append(fact)
        except Exception as e:
